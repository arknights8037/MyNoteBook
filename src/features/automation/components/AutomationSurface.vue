...
async function load(): Promise<void> {
  ...
  error.value = ''
  try {
    const service = await props.getService()
...
async function toggleTask(task: AutomationTask, event: BrowserEvent): Promise<void> {
  ...
  const result = await (await props.getService()).setTaskEnabled(task, enabled)
  if (!result.ok) {
    error.value = result.error.message
    return
  }
...
async function enqueueTask(task: AutomationTask): Promise<void> {
  const result = await (await props.getService()).enqueueTask(task)
...
async function deleteTask(task: AutomationTask): Promise<void> {
  ...
  const result = await (await props.getService()).deleteTask(task.id)
  if (!result.ok) error.value = result.error.message
  await load()
}
