...
export async function exportDocumentToHtml(
  ...
  const body = await nodesToHtml(content.content ?? [])
...
async function nodesToHtml(nodes: JSONContent[]): Promise<string> {
  return (await Promise.all(nodes.map((node) => nodeToHtml(node)))).join('\n')
}

async function nodeToHtml(node: JSONContent): Promise<string> {
  if (node.type === 'heading') {
