...
import { validateImageFile } from '@/editor/io/imageFile'
import { createAssetUrl, parseAssetUrl } from '@/models/documents/asset'
...
async function useFile(file: BrowserFile | undefined): Promise<void> {
  ...
  try {
    const asset = await assetPort().storeFile(file)
...
async function resolveImageSource(src: string | null | undefined): Promise<void> {
  ...
  try {
    resolvedSrc.value = await assetPort().resolveAssetUrl(nextSrc)
  } catch {
    resolvedSrc.value = ''
  }
}
