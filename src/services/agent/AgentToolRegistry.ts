...
export const AGENT_MAX_TOOL_ROUNDS = DEFAULT_AGENT_MAX_TOOL_ROUNDS
export const AGENT_MAX_TOOL_FAILURES = DEFAULT_AGENT_MAX_TOOL_FAILURES
export const AGENT_MAX_TASK_DURATION_MS = DEFAULT_AGENT_MAX_DURATION_MS

type AgentToolSeed = Omit<
  AgentToolDefinition,
  'executionAuthorization' | 'mutationApproval' | 'externalActionApproval' | 'presentation'
> & {
  authorization: 'none' | 'required'
}

export const AGENT_TOOL_REGISTRY: readonly AgentToolDefinition[] = defineAgentTools([
  {
  ...
  },
  {
    name: 'execute_shell',
    description:
      '执行 Runtime 白名单内的只读 Windows 查询、Git/rg 检索或开发工具版本命令；不接受脚本文本。',
    risk: 'read',
    authorization: 'none',
    maxCallsPerRun: 12,
    tags: ['system.inspect'],
  ...
  },
  {
    ...
    description:
      '用户明确要求添加 MCP 服务时，经授权写入停用且未信任的配置草稿；不会连接、启动或暴露工具。',
    risk: 'draft',
    authorization: 'required',
    maxCallsPerRun: 2,
    tags: ['external.may_write'],
...
function defineAgentTools(seeds: readonly AgentToolSeed[]): AgentToolDefinition[] {
  return seeds.map(({ authorization, ...seed }) => ({
    ...seed,
    executionAuthorization: resolveExecutionAuthorization(seed.risk, authorization),
    mutationApproval:
      seed.risk === 'write' && authorization === 'required' ? 'required' : 'not_required',
    externalActionApproval: 'not_required',
    presentation: createPresentation(seed),
  }))
}

function resolveExecutionAuthorization(
  risk: AgentToolDefinition['risk'],
  authorization: AgentToolSeed['authorization'],
): AgentAuthorizationRequirement {
  return risk !== 'write' && authorization === 'required' ? 'required' : 'not_required'
}

function createPresentation(
  tool: Pick<AgentToolDefinition, 'name' | 'tags'>,
): AgentToolPresentationMetadata {
  const firstTag = tool.tags[0] ?? ''
  const category = firstTag.startsWith('document.')
    ? 'document'
    : firstTag.startsWith('knowledge.')
      ? 'knowledge'
      : firstTag.startsWith('system.')
        ? 'system'
        : firstTag.startsWith('cognition.')
          ? 'interaction'
          : 'external'
  return { label: tool.name, category }
}
...
export function resolveAgentOutputTokenLimit(
  ...
  const requested = policy.allowWriteProposals
    ...
    : configuredMaxTokens
  return Math.min(requested, policy.budget?.maxOutputTokens ?? policy.tokenBudget)
}
