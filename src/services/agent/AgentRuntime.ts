...
import type { AgentAuthorizationRequest, AgentTimelineEvent } from '@/models/agent/agentRuntime'
...
import { safeErrorMessage } from '@/services/security/SensitiveDataRedaction'
import type { AgentOutputContract } from '@/services/agent/AgentOutputContract'
import type { AgentDocumentEditProposal } from '@/services/agent/AgentEditProposalGuard'
...
export async function runAgentToolLoop(input: AgentRuntimeInput): Promise<AgentRuntimeResult> {
  const { runAiSdkAgent } = await import('@/services/ai/AiSdkAgentRuntime')
  try {
