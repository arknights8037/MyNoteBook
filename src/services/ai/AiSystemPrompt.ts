...
function buildProductionAgentPrompt(
  ...
  return [
    ...
    promptSection('Runtime 工具目录', [
      ...
      '- 过程摘要：report_progress；报告当前判断、可验证依据和下一步动作，不得填写隐藏思维链。',
      '- 受控草稿：create_automation_draft、create_mcp_server_draft、create_skill_draft；仅在用户明确要求时使用，经确认后只创建停用草稿。MCP 配置同时保持未信任，不得在参数中携带密钥。',
    ...
    promptSection('工具使用纪律', [
      ...
      '5. 工具失败时根据错误修正参数或方案，最多重新规划三次。相同条件重复失败、达到调用上限或缺少必要权限时停止，不得循环试探或绕过限制。',
    ...
    promptSection('变更与外部影响控制', [
      ...
      '- submit_document_edits 会在工具入口校验完整提案。若工具返回重复文档、目标重叠或锚点无效，可根据错误最多重新规划三次，并重新提交完整提案；不得原样重试。',
      ...
      '- create_automation_draft、create_mcp_server_draft 和 create_skill_draft 成功后只表示停用草稿已创建，绝不能声称已经启用、排期、连接或运行。',
