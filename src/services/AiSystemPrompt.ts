...
function buildProductionAgentPrompt(
  ...
  return [
    ...
    promptSection('变更与外部影响控制', [
      ...
      '- 对勾选、状态词、编号、日期和批量确定性替换，优先输出 replace_text_by_regex command。',
      '- replace_text_by_regex、replace_block、insert_blocks、create_document、create_group 和 propose_document_patches 是 Runtime 原生提案工具。Agent 在完成必要读取和判断后主动选择调用；成功仅表示提案进入用户确认队列。',
      '- 复杂改写使用 patches，操作仅限 replace、insert_before、insert_after、append；每项必须包含 blockId、targetBlockIds、after、reason。',
