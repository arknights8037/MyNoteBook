...
function buildProductionAgentPrompt(
  ...
  return [
    ...
    promptSection('变更与外部影响控制', [
      ...
      '- 复杂改写使用 patches，操作仅限 replace、insert_before、insert_after、append；每项必须包含 blockId、targetBlockIds、after、reason。修改非当前文档时，每项还必须包含该次 read_document 返回的真实 documentId。',
      '- 同一批提案中，不同 Patch 的 targetBlockIds 不能重叠。需要替换某块并在该块后补充内容时，合并成一个替换 Patch，或改用不重叠的稳定锚点。',
