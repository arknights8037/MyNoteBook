...
describe('Agent P1 deterministic edit flow', () => {
  it('locates a P0 block without selection and turns a model command into a confirmable patch', () => {
    const blocks = [
    ...
    const response = parseAgentResponse({
      ...
      task: {
        id: 'task-1', sessionId: 'doc-1', status: 'running', userInstruction: '将 P0 事项标记为完成',
        contextScope: 'current_document', model: 'test', currentStep: '生成修改', createdAt: 1,
    ...
    const patches = response.commands.flatMap((command) =>
      createRegexReplacePatches({
        command, taskId: 'task-1', documentId: 'doc-1', expectedVersion: 3, blocks: targetBlocks,
        createId: () => 'patch-1',
      }),
    )
    const applied = applyAgentBlockPatches(
      {
        type: 'doc',
        content: [
          { type: 'paragraph', attrs: { id: 'p0' }, content: [{ type: 'text', text: '[ ] P0 完成可信修改闭环' }] },
          { type: 'paragraph', attrs: { id: 'p1' }, content: [{ type: 'text', text: '[ ] P1 完成工具调用循环' }] },
        ],
      },
      patches,
    )
    ...
    expect(patches).toMatchObject([{ before: '[ ] P0 完成可信修改闭环', after: '[x] P0 完成可信修改闭环' }])
