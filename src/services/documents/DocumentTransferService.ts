...
import { parseNotebookJsonDocument } from '@/editor/io/jsonImport'
...
export class DocumentTransferService {
  constructor(
    ...
    private readonly assetPort?: AssetPort,
  ) {}
  ...
  async prepareExport(source: DocumentExportSource): Promise<PreparedDocumentExport> {
    ...
    const [markdown, html] = await Promise.all([
      ...
      this.converters.toHtml(source.content, metadata, this.assetPort),
    ])
