import type { MindMapContent, MindMapDocument, MindMapSummary } from '@/models/workspace/mindMap'
import { validateMindMapContent } from '@/models/workspace/mindMap'
  ...
  async get(id: string): Promise<AppResult<MindMapDocument>> {
    try {
      const rows = await this.sql.select<MindMapRow>(
        'SELECT * FROM mind_maps WHERE id = ? LIMIT 1',
        [id],
      )
...
function mapRow(row: MindMapRow): AppResult<MindMapDocument> {
  try {
    const content = parseJsonStrict<MindMapContent>(row.content_json, '思维导图数据')
    const validation = validateMindMapContent(content)
    if (validation)
      return err({ code: 'validation-error', message: `思维导图数据损坏：${validation}` })
    return ok({
      id: row.id,
    ...
    return err({
      ...
      message: error instanceof JsonCodecError ? error.message : '思维导图数据无效。',
      cause: error,
    })
  }
}
