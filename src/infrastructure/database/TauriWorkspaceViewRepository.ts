...
import { validateWorkspaceViewPayload, type StructuredWorkspaceView, type StructuredWorkspaceViewPayload, type StructuredWorkspaceViewSummary, type StructuredWorkspaceViewType } from '@/models/workspaceView'
...
export class TauriWorkspaceViewRepository implements WorkspaceViewRepository {
  ...
  async create(input: { id: string; parentId?: string | null; sortOrder?: number; viewType: StructuredWorkspaceViewType; title: string; payload: StructuredWorkspaceViewPayload; createdAt?: number }): Promise<AppResult<StructuredWorkspaceView>> {
    ...
    try {
      await this.sql.execute('INSERT INTO workspace_views (id, parent_id, sort_order, view_type, title, payload_json, schema_version, version, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 1, 1, ?, ?)', [input.id, input.parentId ?? null, input.sortOrder ?? 0, input.viewType, input.title.trim(), JSON.stringify(input.payload), now, now])
      return this.get(input.id)
    } catch (error) { return err(normalizeError(error, '无法创建工作空间视图。')) }
  }
  async get(id: string): Promise<AppResult<StructuredWorkspaceView>> {
    try {
      const rows = await this.sql.select<Row>('SELECT * FROM workspace_views WHERE id = ? LIMIT 1', [id])
      return rows[0] ? map(rows[0]) : err({ code: 'not-found', message: '工作空间视图不存在。' })
    } catch (error) { return err(normalizeError(error, '无法读取工作空间视图。')) }
  }
  ...
  async update(input: { id: string; expectedVersion: number; title: string; payload: StructuredWorkspaceViewPayload; updatedAt?: number }): Promise<AppResult<StructuredWorkspaceView>> {
    ...
    if (invalid) return err({ code: 'validation-error', message: invalid })
    try {
      const result = await this.sql.execute('UPDATE workspace_views SET title = ?, payload_json = ?, version = version + 1, updated_at = ? WHERE id = ? AND version = ?', [input.title.trim(), JSON.stringify(input.payload), input.updatedAt ?? Date.now(), input.id, input.expectedVersion])
      if (result.rowsAffected !== 1) return err({ code: 'revision-conflict', message: '视图已更新，请重新打开。' })
      return this.get(input.id)
    } catch (error) { return err(normalizeError(error, '无法保存工作空间视图。')) }
  }
  async move(input: { id: string; expectedVersion: number; parentId: string | null; sortOrder?: number; updatedAt?: number }): Promise<AppResult<StructuredWorkspaceView>> {
    if (input.id === input.parentId) return err({ code: 'validation-error', message: '视图不能成为自己的子页面。' })
    try {
      const result = await this.sql.execute('UPDATE workspace_views SET parent_id = ?, sort_order = ?, updated_at = ? WHERE id = ? AND version = ?', [input.parentId, input.sortOrder ?? 0, input.updatedAt ?? Date.now(), input.id, input.expectedVersion])
      if (result.rowsAffected !== 1) return err({ code: 'revision-conflict', message: '视图已更新，请重新打开。' })
      return this.get(input.id)
  ...
  async delete(id: string): Promise<AppResult<void>> {
    try {
      ...
      return result.rowsAffected === 1 ? ok(undefined) : err({ code: 'not-found', message: '视图不存在。' })
    } catch (error) { return err(normalizeError(error, '无法删除视图。')) }
  }
}

function validate(title: string, type: StructuredWorkspaceViewType, payload: StructuredWorkspaceViewPayload): string | null {
