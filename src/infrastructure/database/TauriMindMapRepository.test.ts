...
describe('TauriMindMapRepository', () => {
  it('persists aggregate versions, revision history and rejects stale writes', async () => {
    ...
    await expect(repository.move({
      ...
      ok: true,
      value: { parentId: 'folder-1', version: 1 },
    })
    ...
    const updated = await repository.update({
      id: 'map-1', expectedVersion: 1, title: '产品规划', content,
      actorType: 'user', updatedAt: 20,
    })
    expect(updated).toMatchObject({ ok: true, value: { version: 2 } })
    await expect(repository.update({
      id: 'map-1', expectedVersion: 1, title: '旧写入', content,
    ...
    await expect(repository.list()).resolves.toMatchObject({
      ok: true,
      value: [{ id: 'map-1', parentId: 'folder-1', nodeCount: 2, version: 2 }],
    })
