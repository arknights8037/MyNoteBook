    ...
    try {
      await invoke('commit_result_verification', {
        input: {
          ...
          completedAt:
            input.nextStatus === 'completed' ? input.verification.createdAt : null,
          error:
            input.verification.verdict === 'failed' ? input.verification.summary : null,
        },
      })
...
function mapTaskRun(row: Record<string, unknown>): TaskRun {
  return {
    ...
    frozenInput: parseObject(row.frozen_input_json),
    acceptanceCriteria: parseObject(row.acceptance_criteria_json) as AcceptanceCriteria,
