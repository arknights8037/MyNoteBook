...
import { normalizeError, err, ok, type AppResult } from '@/models/shared/result'
...
} from '@/repositories/agent/AgentRepository'
import type { SqlClient } from '@/repositories/shared/SqlClient'
import { parseStringArray, parseVersionedJson } from '@/repositories/shared/jsonCodec'
...
function parseExecutionPolicy(value: string | undefined) {
  ...
  try {
    const parsed = parseVersionedJson<Record<string, unknown> & { version: number }>(value)
    return { ...fallback, ...(parsed ?? {}), version: 1 as const }
  } catch {
    return fallback
  }
}
