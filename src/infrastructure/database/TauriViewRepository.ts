...
import type { ViewDefinition, ViewDependency, ViewGenerationConfig, ViewSnapshot, ViewType, ViewWritebackPolicy } from '@/models/view'
...
export class TauriViewRepository implements ViewRepository {
  ...
  async createDefinition(input: {
    ...
    if (
      ...
      (!input.generation?.prompt.trim() || !input.generation.provider.trim() || !input.generation.model.trim())
    ) {
      return err({ code: 'validation-error', message: 'Generated View 缺少 prompt、Provider 或模型。' })
    }
...
function mapDefinition(row: Record<string, unknown>): ViewDefinition {
  return {
    ...
    scopeQuery: parseObject(row.scope_query_json),
    projectionSchema: row.projection_schema_json ? parseObject(row.projection_schema_json) : null,
    renderSpec: parseObject(row.render_spec_json),
    ...
    overrideContent: parseJson(row.override_content_json),
...
function parseSkillVersions(value: unknown): Array<{ id: string; version: string | null }> {
  ...
  return parsed.flatMap((item) => {
    ...
    return [{ id: (item as { id: string }).id, version: typeof version === 'string' ? version : null }]
  })
}
