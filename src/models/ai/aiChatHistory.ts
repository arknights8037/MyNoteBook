...
function normalizeAgentRuntimeHistory(value: unknown): AgentRuntimeViewState | null {
  ...
  return {
    ...
    lifecycle: normalizeLifecycleState(state.lifecycle),
    runEvents: normalizeRunEvents(state.runEvents),
  }
}

function normalizeLifecycleState(value: unknown): AgentRunLifecycleState {
  const fallback = createIdleAgentRunLifecycle()
  if (!value || typeof value !== 'object') return fallback
  const state = value as Partial<AgentRunLifecycleState>
  const phases = [
    'idle',
    'planning',
    'executing',
  ...
  if (!phases.includes(state.phase ?? '')) return fallback
  return {
    id: typeof state.id === 'string' ? state.id : null,
    goal: typeof state.goal === 'string' ? state.goal : '',
    phase: state.phase as AgentRunLifecycleState['phase'],
    plan: Array.isArray(state.plan) ? state.plan.slice(0, 64) : [],
    currentStepId: typeof state.currentStepId === 'string' ? state.currentStepId : null,
    pendingApproval: null,
    activeToolCallId:
      typeof state.activeToolCallId === 'string' ? state.activeToolCallId : null,
    error: typeof state.error === 'string' ? state.error : null,
    startedAt: Number.isFinite(state.startedAt) ? Number(state.startedAt) : null,
    completedAt: Number.isFinite(state.completedAt) ? Number(state.completedAt) : null,
  }
}

function normalizeRunEvents(value: unknown): AgentRunEvent[] {
  if (!Array.isArray(value)) return []
  return value
    .filter(
      (event): event is AgentRunEvent =>
        Boolean(event) &&
        typeof event === 'object' &&
        typeof (event as Partial<AgentRunEvent>).type === 'string' &&
        Number.isFinite((event as Partial<AgentRunEvent>).occurredAt),
    )
    .slice(0, 256)
}
