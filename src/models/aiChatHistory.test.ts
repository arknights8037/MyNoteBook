import { beforeEach, describe, expect, it } from 'vitest'

import {
  AI_CHAT_HISTORY_LIMIT,
  DEFAULT_AGENT_PROJECT_ID,
  normalizeAgentWorkspaceHistory,
  purgeLegacyAgentHistoryStorage,
  UNGROUPED_AGENT_PROJECT_ID,
} from './aiChatHistory'

describe('Agent workspace history model', () => {
  beforeEach(() => globalThis.localStorage.clear())

  it('normalizes the versioned project and conversation state', () => {
    const state = normalizeAgentWorkspaceHistory({
      version: 2,
      projects: [
        {
          id: 'project-1',
          name: 'Research',
          workspaceRootIds: ['group-1', 'group-1'],
          pinnedAt: 30,
          createdAt: 10,
          updatedAt: 20,
        },
      ],
      activeProjectId: 'project-1',
      items: [historyItem('history-1', 'project-1')],
    })

    expect(state.activeProjectId).toBe('project-1')
    expect(state.projects[0]?.workspaceRootIds).toEqual(['group-1'])
    expect(state.projects[0]?.pinnedAt).toBe(30)
    expect(state.items[0]).toMatchObject({
      id: 'history-1',
      projectId: 'project-1',
      messageCount: 1,
    })
  })

  it('rejects unversioned legacy arrays instead of migrating them', () => {
    const state = normalizeAgentWorkspaceHistory([historyItem('legacy', 'project-1')])

    expect(state.activeProjectId).toBe(DEFAULT_AGENT_PROJECT_ID)
    expect(state.items).toEqual([])
  })

  it('purges both legacy local history keys', () => {
    localStorage.setItem('my-notebook:ai-chat-history', 'legacy')
    localStorage.setItem('my-notebook:agent-workspace-history:v2', 'legacy-v2')

    purgeLegacyAgentHistoryStorage()

    expect(localStorage.length).toBe(0)
  })

  it('limits conversations and removes records for unknown projects', () => {
    const state = normalizeAgentWorkspaceHistory({
      version: 2,
      projects: [project('project-1')],
      activeProjectId: 'project-1',
      items: [
        ...Array.from({ length: AI_CHAT_HISTORY_LIMIT + 5 }, (_, index) =>
          historyItem(`history-${index}`, 'project-1'),
        ),
        historyItem('orphan', 'missing-project'),
      ],
    })

    expect(state.items).toHaveLength(AI_CHAT_HISTORY_LIMIT)
    expect(state.items.some((item) => item.id === 'orphan')).toBe(false)
  })

  it('preserves ungrouped tasks without creating a synthetic project', () => {
    const state = normalizeAgentWorkspaceHistory({
      version: 2,
      projects: [project('project-1')],
      activeProjectId: UNGROUPED_AGENT_PROJECT_ID,
      items: [historyItem('loose-task', UNGROUPED_AGENT_PROJECT_ID)],
    })

    expect(state.activeProjectId).toBe(UNGROUPED_AGENT_PROJECT_ID)
    expect(state.projects).toHaveLength(1)
    expect(state.items[0]?.projectId).toBe(UNGROUPED_AGENT_PROJECT_ID)
  })

  it('preserves the parent conversation for an A2A branch', () => {
    const item = historyItem('branch-1', 'project-1')
    item.parentConversationId = 'conversation-1'
    const state = normalizeAgentWorkspaceHistory({
      version: 3,
      projects: [project('project-1')],
      activeProjectId: 'project-1',
      items: [item],
    })

    expect(state.items[0]?.parentConversationId).toBe('conversation-1')
  })

  it('restores structured research results and their run provenance', () => {
    const item = historyItem('research-task', 'project-1')
    item.messages = [
      {
        id: 'research-message',
        role: 'assistant',
        mode: 'agent',
        content: '调研完成',
        status: 'done',
        researchResult: {
          summary: '调研摘要',
          items: [],
          relations: [],
          unresolvedQuestions: ['待确认问题'],
        },
        cognitiveProvenance: {
          sessionId: 'session-1',
          runId: 'run-1',
          modeId: 'research',
          modeVersion: 1,
          templateId: 'research-conclusions',
          templateVersion: 1,
          outputContractId: 'research-result',
          createdAt: 10,
        },
        researchCandidates: [
          {
            itemId: 'C1',
            candidateId: 'candidate-1',
            version: 2,
            decision: 'kept',
            sourceState: 'unverified',
            title: '候选结论',
            content: '候选内容',
            error: '',
          },
        ],
      },
    ]
    const state = normalizeAgentWorkspaceHistory({
      version: 2,
      projects: [project('project-1')],
      activeProjectId: 'project-1',
      items: [item],
    })

    expect(state.items[0]?.messages[0]?.researchResult?.unresolvedQuestions).toEqual(['待确认问题'])
    expect(state.items[0]?.messages[0]?.cognitiveProvenance?.outputContractId).toBe(
      'research-result',
    )
    expect(state.items[0]?.messages[0]?.researchCandidates?.[0]).toMatchObject({
      candidateId: 'candidate-1',
      version: 2,
      decision: 'kept',
    })
  })

  it('restores the completed Agent execution trace with its message', () => {
    const item = historyItem('agent-trace', 'project-1')
    item.messages = [
      {
        id: 'assistant-trace',
        role: 'assistant',
        mode: 'agent',
        content: '任务完成',
        status: 'done',
        agentRuntime: {
          status: 'completed',
          phase: 'completed',
          detail: '任务已完成',
          startedAt: 1_000,
          completedAt: 62_000,
          rounds: 3,
          toolCalls: [],
          timelineEvents: [
            {
              id: 'decision-1',
              kind: 'decision',
              status: 'completed',
              detail: '已完成检索。',
              occurredAt: 2_000,
              completedAt: 3_000,
            },
          ],
          authorizationRequest: null,
          summary: '已完成检索并汇总结论。',
        },
      },
    ]

    const state = normalizeAgentWorkspaceHistory({
      version: 3,
      projects: [project('project-1')],
      activeProjectId: 'project-1',
      items: [item],
    })

    expect(state.items[0]?.messages[0]?.agentRuntime).toMatchObject({
      status: 'completed',
      startedAt: 1_000,
      completedAt: 62_000,
      rounds: 3,
      summary: '已完成检索并汇总结论。',
    })
  })

  it('restores structured Review issues without turning them into candidates', () => {
    const item = historyItem('review-task', 'project-1')
    item.messages = [
      {
        id: 'review-message',
        role: 'assistant',
        mode: 'agent',
        content: '审阅完成',
        status: 'done',
        reviewResult: {
          summary: '发现问题',
          issues: [
            {
              id: 'M1',
              issueType: 'missing_source',
              severity: 'warning',
              title: '缺少来源',
              explanation: '结论没有来源。',
              affectedText: '原结论',
              suggestedAction: '补充来源。',
              sources: [],
              sourceState: 'unverified',
            },
          ],
          unresolvedQuestions: [],
        },
        cognitiveProvenance: {
          sessionId: 'review-session',
          runId: 'review-run',
          modeId: 'review',
          modeVersion: 1,
          templateId: 'review-findings',
          templateVersion: 1,
          outputContractId: 'review-result',
          createdAt: 20,
        },
      },
    ]
    const state = normalizeAgentWorkspaceHistory({
      version: 2,
      projects: [project('project-1')],
      activeProjectId: 'project-1',
      items: [item],
    })

    expect(state.items[0]?.messages[0]?.reviewResult?.issues[0]).toMatchObject({
      issueType: 'missing_source',
      sourceState: 'unverified',
    })
    expect(state.items[0]?.messages[0]?.researchCandidates).toBeUndefined()
  })

  it('restores Learning attempts and the waiting prompt with the conversation', () => {
    const item = historyItem('learning-task', 'project-1')
    item.messages = [
      {
        id: 'learning-message',
        role: 'assistant',
        mode: 'agent',
        content: '继续学习',
        status: 'done',
        learningResult: {
          phase: 'waiting_user',
          feedback: { correctPoints: ['正确点'], omissions: [], misconceptions: [] },
          understandingState: 'partial',
          evidence: '用户给出了可见解释。',
          nextPrompt: { kind: 'guided_question', content: '下一题', hintLevel: 0 },
          candidateUnderstanding: null,
        },
        learningState: {
          version: 1,
          topic: '事件循环',
          currentPrompt: '下一题',
          promptKind: 'guided_question',
          hintLevel: 0,
          attempts: [
            {
              id: 'attempt-1',
              response: '我的解释',
              feedback: { correctPoints: ['正确点'], omissions: [], misconceptions: [] },
              understandingState: 'partial',
              evidence: '用户给出了可见解释。',
              createdAt: 10,
            },
          ],
          understandingState: 'partial',
          nextStep: 'continue',
        },
        cognitiveProvenance: {
          sessionId: 'learning-session',
          runId: 'learning-run',
          modeId: 'learning',
          modeVersion: 1,
          templateId: 'learning-coach',
          templateVersion: 1,
          outputContractId: 'learning-turn',
          createdAt: 20,
        },
      },
    ]
    const state = normalizeAgentWorkspaceHistory({
      version: 2,
      projects: [project('project-1')],
      activeProjectId: 'project-1',
      items: [item],
    })

    expect(state.items[0]?.messages[0]?.learningState).toMatchObject({
      currentPrompt: '下一题',
      attempts: [{ id: 'attempt-1', response: '我的解释' }],
    })
    expect(state.items[0]?.messages[0]?.cognitiveProvenance?.modeId).toBe('learning')
  })
})

function project(id: string) {
  return {
    id,
    name: 'Project',
    workspaceRootIds: [],
    pinnedAt: null,
    createdAt: 1,
    updatedAt: 1,
  }
}

function historyItem(id: string, projectId: string) {
  return {
    id,
    projectId,
    parentConversationId: null as string | null,
    title: 'Conversation',
    updatedAt: 1,
    messageCount: 1,
    provider: 'openai',
    model: 'gpt',
    pinnedAt: null,
    messages: [
      {
        id: `${id}-message`,
        role: 'user',
        mode: 'agent',
        content: 'work',
        status: 'done',
      },
    ],
  }
}
