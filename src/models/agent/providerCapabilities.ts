...
export function resolveProviderCapabilities(
  ...
  if (provider === 'anthropic')
    return { ...BASE, reasoningEffort: true, reasoningContent: true, maxContextTokens: 200_000 }
  if (provider === 'deepseek') {
    const legacyReasoning =
      normalized.includes('reasoner') ||
      normalized.includes('thinking') ||
      normalized.includes('r1')
    const v4Pro = /(?:^|[-_.\s])v4[-_.\s]*pro(?:$|[-_.\s])/.test(normalized)
    const reasoning = legacyReasoning || v4Pro
    return {
      ...BASE,
      reasoningEffort: reasoning,
      reasoningContent: reasoning,
      toolChoice: !legacyReasoning,
      maxContextTokens: 128_000,
    }
  }
  if (provider === 'qwen')
    return { ...BASE, reasoningEffort: true, reasoningContent: true, structuredOutput: true }
