...
class Client implements SqlClient {
  ...
  async select<T extends Record<string, unknown>>(
    ...
    return this.database
      .prepare(sql)
      .all(...values.map((value) => (typeof value === 'boolean' ? Number(value) : value))) as T[]
  }
}
...
describe('TauriWorkspaceViewRepository', () => {
  it('persists typed payloads, revisions and optimistic versions', async () => {
    ...
    client.database.exec(
      readFileSync(
        join(process.cwd(), 'src-tauri/migrations/0021_add_workspace_views.sql'),
        'utf8',
      ),
    )
    client.database.exec(
      readFileSync(
        join(process.cwd(), 'src-tauri/migrations/0023_add_workspace_view_tree_position.sql'),
        'utf8',
      ),
    )
    client.database.exec(
      readFileSync(
        join(process.cwd(), 'src-tauri/migrations/0026_add_workspace_view_pinning.sql'),
        'utf8',
      ),
    )
    ...
    ).toMatchObject({ ok: true, value: { parentId: 'folder-1', version: 1 } })
    const updated = await repository.update({
      id: 'view-1',
      expectedVersion: 1,
      title: '流程',
      payload: { ...payload, source: 'flowchart LR\n  a[开始] --> b[完成]' },
      updatedAt: 2,
    })
    ...
    expect(
      client.database.prepare('SELECT count(*) count FROM workspace_view_revisions').get(),
    ).toEqual({ count: 2 })
    expect(
      await repository.move({ id: 'view-1', expectedVersion: 2, parentId: null, updatedAt: 3 }),
    ).toMatchObject({ ok: true, value: { parentId: null, version: 3 } })
    expect(
    ...
    expect(await repository.setPinned({ id: 'view-1', pinnedAt: 4 })).toMatchObject({
      ok: true,
      value: { pinnedAt: 4 },
    })
    expect(await repository.list()).toMatchObject({
      ok: true,
      value: [{ id: 'view-1', pinnedAt: 4 }],
    })
