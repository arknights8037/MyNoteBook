...
describe('TauriWorkspaceViewRepository', () => {
  it('persists typed payloads, revisions and optimistic versions', async () => {
    ...
    expect(await repository.move({ id: 'view-1', expectedVersion: 2, parentId: null, updatedAt: 3 })).toMatchObject({ ok: true, value: { parentId: null, version: 3 } })
    expect(await repository.move({ id: 'view-1', expectedVersion: 2, parentId: 'folder-2', updatedAt: 4 })).toMatchObject({ ok: false, error: { code: 'revision-conflict' } })
    expect(client.database.prepare('SELECT count(*) count FROM workspace_view_revisions').get()).toEqual({ count: 3 })
