...
P4.5 已冻结 Event Envelope v1，并为 Outbox 增加有界退避与 Dead Letter 字段。Timer/Outbox 的领取和结算不再由 WebView 调用；活动与审计页面可按 correlation ID 追踪 Workflow wait、Timer、Domain Event 与 Outbox。真正消费事件并启动后续 Run 的 dispatcher 仍未实现。
