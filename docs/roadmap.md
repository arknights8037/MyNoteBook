...
更广泛的 Signal/消息连接、邮件 OAuth、外部日历账户和网页裁剪仍属于未来集成。信息首页已提供本地日程卡片，但它不是外部日历连接器。标准 IMAP 邮箱读取已按[邮箱连接器与收件箱](email-inbox.md)完成首个只读版本；RSS 已按[RSS 连接器与收件箱](rss-inbox.md)完成手动条件同步与统一收件箱版本；钉钉已按[钉钉消息连接器与收件箱](dingtalk-inbox.md)完成企业机器人 Stream 只读接收版本。后续接入顺序应遵循：
