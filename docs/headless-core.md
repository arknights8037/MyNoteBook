...
本文记录 Phase 6 的当前实现事实、协议安全边界和后续迁移顺序。Phase 6 尚未完成；当前已落地独立 Core 控制进程、Desktop 发现协议、WebView 数据库 prepare/query/mutation catalog，以及不依赖 Desktop 生命周期的 Durable Timer 调度器。Workflow dispatcher、Connector、Action 和其他 Rust 数据库所有权仍在后续迁移中。
...
| `POST /v1/database/prepare`           | 创建或升级数据库，并启动该目录的 Core Durable Timer          |
...
| `POST /v1/database/close-pool`        | 停止匹配的 Timer 并关闭指定目录的 Core 读写池                 |
| `POST /v1/timer/snapshot`             | 返回 Core 持有的 Timer 健康与积压快照                        |
| `POST /v1/timer/quiesce`              | 中止并等待 Core Timer 调度任务，用于数据目录迁移              |
| `POST /v1/timer/resume`               | 在迁移成功的目标目录或失败回滚的原目录恢复 Core Timer         |
...
2. 将 Durable Timer、Outbox、Workflow、Automation、Signal 和 Connector watcher 移入 Core。（Durable Timer 已迁移）
...
- Core loopback 集成测试会准备真实数据库、插入到期 Timer，并验证 Core 独立触发 Domain Event/Outbox 后可读取健康快照。
