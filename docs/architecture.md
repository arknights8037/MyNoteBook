...
- `agent_worker_supervisor.rs`：Phase 3 Worker 子进程身份、NDJSON 通道、heartbeat、重启、活动/待授权/待领取终态的脱敏快照、显式终态 ACK、崩溃终态、Provider 流式代理，以及全部内置 Domain Tool/MCP 的受控分发；当前为可选 Runtime，尚未成为默认生产 Agent。
