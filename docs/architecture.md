...
- 默认 Agent Runtime 的规划、模型循环、工具调度与标准 Patch proposal 编译已移入 sidecar；Rust 会先持久化这些 proposal，再允许 Vue 显示 Diff。授权 UI 仍由 Vue 投影，A2A 与 Cognitive 的队列决策和终态状态机仍依附 WebView 恢复路径。普通 Run 仍没有 durable checkpoint/resume。
