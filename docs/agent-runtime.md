...
当前生产 Runtime 位于 Rust 托管的 Node sidecar。`useAgentRun` 冻结交互快照，通过 `TauriAgentRuntimeAdapter` 提交、取消和转发授权回复，并把 Runtime 事件投影到现有 UI；WebView 版 `AiSdkAgentRuntimeAdapter` 仅由显式兼容配置启用。
