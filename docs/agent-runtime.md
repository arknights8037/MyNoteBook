...
`useAgentRun` 只负责连接一次交互式运行的准备、执行、收敛和终态。`prepareAgentRun()` 冻结输入、恢复 Cognitive Session 并创建任务；`AgentRunEngine` 接受 command、产生 event，并用纯 reducer 更新稳定生命周期；`prepareAgentRunExecution()` 冻结模型执行输入，最后由 `runAgentToolLoop` 将 Agent 模式交给 `AiSdkAgentRuntime` 的 AI SDK `ToolLoopAgent`。
...
### 1.1 Run、Plan、Action、Domain、Event

顶层 Run 只表达稳定且互斥的生命周期：`idle / planning / executing / waiting_user / waiting_external / completed / failed / cancelled`。具体执行内容保存在 `AgentPlanStep`，步骤状态为 `pending / running / succeeded / failed / blocked`，下一步由依赖满足情况计算，不通过复合枚举硬编码。
...
- `AgentTask` 与 `AgentToolCall` 的执行/审计状态。
- `AgentPatchSet` 的 proposed、approved、applied、rejected 业务状态。
- `CognitiveSession` 的 active、waiting_user、completed 状态。
- Delegation/A2A 的发送、响应、接受和提交状态。

父 Run 只记录当前步骤依赖这些子对象，不展开它们的协议状态。
...
文档目标与项目分组选择器共用宽容的本地匹配：统一处理大小写、全半角、重音、空格和标点，支持紧凑查询与有界有序缩写，并按标题、分组路径和补充元数据排序。文档选项会显示所属分组路径；分组也可通过其后代页面标题定位。
