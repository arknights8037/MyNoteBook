...
本文最初用于 C1.5-R 的能力审查。下文按 2026-07-28 主干代码重新核对，保留问题的关闭状态和仍存在的设计债，不再把历史实现阶段描述成当前状态。
...
这组关闭不意味着可以放松安全边界：工具执行仍必须先写 `running` 审计，取消仍需等待已开始调用写入终态，MCP 的信任与 `readOnlyHint` 仍需同时满足。
